import { isArray, isStrOrNum } from '../shared/utils'
...
export function reconcileChildren(returnFiber, children) {
  ...
  const oldFiber = returnFiber.alternate?.child // 上一个 fiber 对象对应的旧 fiber 对象
  let i = 0 // 记录 children 数组的索引（下标）
  let lastPlacedIndex = 0 // 上一次 DOM 节点插入的最远位置
  // 是否需要追踪副作用，该变量是一个布尔值
  // true 代表组件更新
  // false 代表组件初次渲染
  const shouldTrackSideEffects = !!returnFiber.alternate // 是否需要追踪副作用
